...
Using the features of functional programming should lead to less code which in turn usually leads to less bugs and better understandability. \par


Scala  occuppies a worthy 14th position in 'RedMonk Programming Language Rankings' \cite{redmonk:1}; Scala developers benefit from a growing Scala community, a lot of online materials addressed to both novices and advanced developers (introduction online courses, step-by-step guides, Twitter's Scala guide etc.) . There are general guidelines on how to approach one problem in a functional perspective, with parallels on OO programming, there are even automated tools for style checking Scala code (see ScalaStyle). Yet Scala language supports both Object Oriented and Functional Programming paradigm.  




yet when you ask the question: `Is this Scala code making the most out of the functional programming paradigm? ` it's really difficult to find an answer, at least if you don't have an FP expert nearby. One solution to this problem with be an automatic tool checker that would be able to asses one's code `functional-ness' and to guide the Scala programmer through the coding process and help him understand what features of FP he is using or missing on. But what means that a code is functional? How can we figure that out? What are the specifics of a functional code? Our study would like begin with lookin for such kind of FP propertises and then try to figure out and propose a set of metrics that would check on FP-like properties.\par
